    ...
    async def _force_conclusion(
        ...
        try:
            ...
            await publish_session_progress_update(
                ...
                stage_execution_id=getattr(context.agent, '_current_stage_execution_id', None),
                parent_stage_execution_id=parallel_meta.parent_stage_execution_id if parallel_meta else None,
                parallel_index=parallel_meta.parallel_index if parallel_meta else None,
                agent_name=parallel_meta.agent_name if parallel_meta else None
            )
        except Exception as e:
            if logger:
