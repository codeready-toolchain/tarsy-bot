...
class HistoryService:
    ...
    async def fail_active_stages_on_timeout(
        self,
        session_id: str,
        timeout_seconds: int
    ) -> int:
        """
        Mark all active stages as failed when session times out.
        
        Creates detailed error messages with timing context to help diagnose
        whether stages legitimately took too long or didn't have enough time.
        
        Args:
            session_id: Session ID that timed out
            timeout_seconds: Session timeout value in seconds
