...
class ScoringService:
    ...
    async def _update_score_status(
        self, score_id: str, status: str, completed_at_us: Optional[int] = None
    ) -> bool:
        """
        ...
        if existing_score:
            if not force_rescore:
                ...
                if existing_score.status in ScoringStatus.active_values():
                    raise ValueError(
                        f"Cannot force rescore while an already existing scoring is {existing_score.status}"
                    )
    ...
    async def _execute_scoring(self, score_id: str, session_id: str) -> None:
        ...
        try:
            ...
            await self._update_score_status(score_id, ScoringStatus.IN_PROGRESS.value)
