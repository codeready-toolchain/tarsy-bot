...
class GeminiNativeThinkingClient:
    ...
    async def generate(
        ...
        Includes retry logic for empty responses (no content AND no tool calls),
        matching the behavior of LLMClient for consistency.
        ...
        Args:
            ...
            parallel_metadata: Optional parallel execution metadata for streaming
            interaction_type: Optional interaction type override
            max_retries: Maximum retry attempts for empty responses (default 3)
        ...
        async with llm_interaction_context(session_id, request_data, stage_execution_id, native_tools_config) as ctx:
            for attempt in range(max_retries + 1):
                try:
                    ...
                    # Thinking content alone is NOT sufficient - we need actionable content or tool calls
                    is_empty = (not accumulated_content or accumulated_content.strip() == "") and len(tool_calls) == 0
                    
                    if is_empty:
                        if attempt < max_retries:
                            logger.warning(
                                f"[{request_id}] Empty LLM response (attempt {attempt + 1}/{max_retries + 1}), "
                                f"retrying in 3s"
                            )
                            ...
                            logger.warning(f"[{request_id}] Empty LLM response on final attempt, injecting error message")
                            accumulated_content = (
                                f"⚠️ **LLM Response Error**\n\n"
                                f"The {self.model} LLM returned empty responses after {max_retries + 1} attempts. "
                                f"This may be due to:\n"
                                f"- Temporary provider issues\n"
                                f"- API rate limiting\n"
                                f"- Model overload\n\n"
                                f"Please try processing this alert again in a few moments."
                            )
                    ...
                    # Don't retry on timeout - let it propagate
                    ...
                    # Don't retry on other exceptions - let them propagate
